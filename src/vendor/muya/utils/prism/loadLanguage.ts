...
// Vite 环境下按需加载 prism 语法包。原版通过 `../../../node_modules/...` 的
// 相对路径动态 import（依赖 marktext 仓库的目录层级），vendor 到应用内后层级
// 变化，改用 import.meta.glob 显式映射（dev 与 build 均为按需懒加载）。
const PRISM_LANGUAGE_MODULES = import.meta.glob(
...
function initLoadLanguage(Prism: IPrismLike) {
    ...
    // If no argument is passed, load all components
        ...
        const loadComponent = async (lang: string): Promise<void> => {
            ...
            const loader = PRISM_LANGUAGE_MODULES[
