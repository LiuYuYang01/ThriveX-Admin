...
export default function RecordCommentPanel({ initRecordId, onClearRecordFilter }: Props) {
  ...
  const [selected, setSelected] = useState<RecordComment | null>(null);
  ...
  const buildQuery = useCallback(
    (values?: RecordCommentFilterQueryParams & { createTime?: [Dayjs, Dayjs] }) => {
  ...
  const getCommentList = useCallback(
    async (values?: RecordCommentFilterQueryParams & { createTime?: [Dayjs, Dayjs] }) => {
      try {
  ...
  const onFilterChange = useCallback(
    async (values: RecordCommentFilterQueryParams & { createTime?: [Dayjs, Dayjs] }) => {
      try {
        setLoading(true);
        const { data } = await getRecordCommentListAPI(buildQuery(values));
        const tree = buildCommentTree(data.result ?? []);
        setList(tree);
        setSelected((prev) => {
  ...
  const { onValuesChange: onFilterValuesChange } = useDebouncedChange<
    RecordCommentFilterQueryParams & { createTime?: [Dayjs, Dayjs] }
  >({
  ...
  const delCommentData = useCallback(
    ...
    [filterForm, getCommentList, selected?.id],
  );

  const openReply = useCallback((record: RecordComment) => {
    setSelected(record);
    setReplyInfo('');
  ...
  const columns: ColumnsType<RecordComment> = useMemo(
    () => [
      commentExpandColumn,
      {
        title: '评论者',
        ...
        key: 'name',
        width: 168,
        render: (_: string, record: RecordComment) => (
          <button
            type="button"
            onClick={(e) => {
              e.stopPropagation();
      ...
      },
      {
        ...
        key: 'content',
        ellipsis: true,
        render: (text: string, record: RecordComment) =>
          text ? (
  ...
  const DetailPanel = ({ record }: { record: RecordComment }) => (
